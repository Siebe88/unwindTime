...
export const favoRelaxMethodsSlice = createSlice({
  ...
  reducers: {
    addNewFavoArray: (
      state: RelaxOption[],
      action: PayloadAction<RelaxOption[]>
    ) => {
    ...
    switchFavo: (
      state: RelaxOption[] | void,
      action: PayloadAction<RelaxOption>
    ) => {
      if (state?.some((method) => method.name === action.payload.name)) {
        return state.filter(
          (method) => method.name !== action.payload.name
        ) as unknown as any;
      } else {
        state?.push(action.payload);
      }
...
export const { addNewFavoArray, addFavo, deleteFavo, switchFavo } =
  favoRelaxMethodsSlice.actions;
