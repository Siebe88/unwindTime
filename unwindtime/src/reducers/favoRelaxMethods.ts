...
export const favoRelaxMethodsSlice = createSlice({
  ...
  reducers: {
    addNewFavoArray: (state: RelaxMethods[], action: PayloadAction<RelaxMethods[]>) => {
      action.payload.forEach((methodFavo) => {
        if (!state.some((method) => method.name === methodFavo.name)) {
          console.log('methodFavo.name', methodFavo);
          //TODO: fix this -> avoid directly pushing and mutating the state - fix it
          // state = [...state, methodFavo]; <- not working
    ...
    // },
    toggleFavo: (state: RelaxMethods[], action: PayloadAction<RelaxMethods>) => {
      // probably the filter is the reason of the error check it later. Find a way to do it without filter
      if (state?.some((method) => method.name === action.payload.name)) {
